...
static int __universalHash(int key) {
    return floor(TABLE_SIZE * fmod((key * ((sqrt(5) - 1) / 2)), 1.0));
}

static int universalHash(int key) {
    const int p = 15485863; // Ближайшее простое число к 2^24 * (sqrt(5) - 1)
    const int a = 7;
    const int b = 13;
...
int main() {
    ...
    int choice;
    int key, value;
    int moduloCollisions = 0, universalCollisions = 0;

    do {
        system("cls");
        cout << "Количество коллизий (модульное хеширование): " << moduloCollisions << endl;
        cout << "Количество коллизий (универсальное хеширование): " << universalCollisions << endl << endl;
        ...
        cout << "----------------------------------------------\n";
        cout << "4. Добавить элемент  (универсальное хеширование)\n";
        cout << "5. Найти элемент     (универсальное хеширование)\n";
        cout << "6. Удалить элемент   (универсальное хеширование)\n";
        cout << "----------------------------------------------\n";
        cout << "0. Выйти\n\n";
        ...
        switch (choice) {
            ...
            case 4:
                cout << "Введите ключ и значение для добавления (через пробел): ";
                cin >> key >> value;
                insertItem(hashTable, key, value, universalHash, universalCollisions);
                break;
            case 5:
                cout << "Введите ключ для поиска: ";
                cin >> key;
                {
                    int values[TABLE_SIZE];
                    int count = searchItems(hashTable, key, universalHash, values);
                    if (count > 0) {
                        cout << "Найденные значения: ";
                        for (int i = 0; i < count; ++i) {
                            cout << values[i] << " ";
            ...
            case 6:
                cout << "Введите ключ для удаления: ";
                cin >> key;
                removeItem(hashTable, key, universalHash, universalCollisions);
                break;
            case 0:

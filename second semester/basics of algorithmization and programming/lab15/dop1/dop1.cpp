...
static void countingSort(int* arr, int n) {
    ...
    // определение индексов элементов arr в массиве count
    for (int i = 0; i < n; ++i) { count[arr[i] - minVal]++; }

    int index = 0;
    for (int i = range - 1; i >= 0; i--) {
        for (int j = 0; j < count[i]; j++) { arr[index++] = i + minVal; }
    }
...
int main() {
    ...
    cout << endl << endl << "Массив C до сортировки подсчётом: ";
    for (int i = 0; i < sizeC; ++i) { cout << arrayC[i] << " "; }

    countingSort(arrayC, sizeC);

    cout << endl << endl << "Массив C после сортировки подсчётом: ";
...
/*int main() {
    ...
    for (int size : sizes) {
        ...
        cout << "Cортировка подсчётом для n = " << size << " заняла " << duration.count() / 1000.0 << " миллисекунд." << endl;
